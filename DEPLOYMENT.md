...
* Use the installer-template from the `template` directory
    ...
    - copy the contents of the prepared directory into the `packages\org.aksdhama0067.tearscope\data` directory
    - bump up the version in the `installer\packages\org.aksdhama0067.tearscope\meta\package` file
* Open the **Qt 5.15.2 (MinGW 810 64-bit)** command line tool and `cd` into the template directory
    - run `binarycreator --offline-only -c config/config.xml -p packages TearScopeInstaller`
